...
class LoginAndRegister extends StateNotifier<AuthState> {
  ...
  Future<void> signInWithEmailAndPassword(String email, String password) async {
    final Userr? user = await _authRepository.signInWithEmailAndPassword(email, password);
    state = AuthState(user: user);
  }

  Future<void> createUserWithEmailAndPassword(String name , String email, String password) async {
    final Userr? user = await _authRepository.createUserWithEmailAndPassword(name, email, password);
    state = AuthState(user: user);
  }
